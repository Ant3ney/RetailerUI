 ...
 */ function getURLFromSanityRefrence(refrence, client) {
    if (!refrence)
        return exicuteFail('getURLFromSanityRefrence: refrence is null');
    if (!refrence.asset)
        return exicuteFail('getURLFromSanityRefrence: refrence.source is null');
    if (!refrence.asset._ref)
        return exicuteFail('getURLFromSanityRefrence: refrence.source.id is null');
    if (!client)
        return exicuteFail('getURLFromSanityRefrence: client is null');
    return new Promise(function (res, rej) {
        var ref = refrence.asset._ref;
        var query = "*[_id==\"".concat(ref, "\"][0]{url}");
        client.fetch(query).then(function (queriedURL) {
            if (!queriedURL)
                return rej('getURLFromSanityRefrence: Sanity query result returned null');
